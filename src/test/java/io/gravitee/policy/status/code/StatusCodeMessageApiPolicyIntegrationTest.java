...
public class StatusCodeMessageApiPolicyIntegrationTest extends AbstractPolicyTest<StatusCodePolicy, StatusCodePolicyConfiguration> {
    ...
    public void configureEndpoints(Map<String, EndpointConnectorPlugin<?, ?>> endpoints) {
        endpoints.putIfAbsent("http-get", EndpointBuilder.build("http-get", HttpPostEntrypointConnectorFactory.class));
        endpoints.putIfAbsent("http-post", EndpointBuilder.build("http-post", HttpPostEntrypointConnectorFactory.class));
    }
    ...
    @Test
    void should_transform_status_code_202_to_200_for_post_request(HttpClient client) {
        wiremock.stubFor(post("/endpoint").willReturn(aResponse().withStatus(202).withBody("Response body")));
        int expectedStatusCode = 200;
